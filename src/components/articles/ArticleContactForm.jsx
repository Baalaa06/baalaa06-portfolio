...
import { submitContactForm } from "/src/firebase/firestore.js"
...
function ArticleContactFormContent({ dataWrapper, selectedItemCategoryId, setShouldHideTitle }) {
    ...
    const _onSubmit = async (e) => {
        ...
        // Submit to Firestore
        const firestoreResponse = await submitContactForm({
            name: name,
            email: email,
            subject: subject,
            message: message
        ...
        if(!fakeEmailRequests && dataWrapper.settings.emailJsPublicKey) {
            ...
            apiResponse = { success: true } // Default to success if no email service
        }

        feedbacks.setActivitySpinnerVisible(false, dataWrapper.uniqueId)
        
        // Consider success if Firestore submission worked (email is optional)
        _onApiResponse(firestoreResponse.success)
