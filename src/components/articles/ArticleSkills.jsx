...
function ArticleSkillsItem({ itemWrapper }) {
    ...
    return (
        <div className={`article-skills-item`} style={{display: 'flex', flexDirection: 'column', padding: '1rem', border: '1px solid #e0e0e0', borderRadius: '8px', marginBottom: '1rem'}}>
...
function ArticleSkillsItemInfo({ itemWrapper }) {
    ...
    useEffect(() => {
        if (location.getActiveSection()?.id === itemWrapper.articleWrapper.sectionId) {
            setAnimationPercentage(percentage || 0)
        }
    }, [location.getActiveSection(), percentage])
        ...
        <div className={`article-skills-item-info`}>
            <div className={`article-skills-item-title text-5`}>
                ...
                <div className={`article-skills-item-title-right-column`}>
                    {hasPercentage && (
                        <span className={`article-skills-item-title-percentage text-3`}>
                            {percentage}%
                        </span>
                    )}
                </div>
