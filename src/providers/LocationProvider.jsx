...
function LocationProvider({ children, sections, categories }) {
    ...
    const goToSection = (section) => {
        if(!section || activeSectionId === section.id)
            return
        // For home section, use clean root URL
        if(section.id === 'about') {
    ...
    const _onHashEvent = () => {
        ...
        // If no hash or root path, default to first section
        if (!hash || hash === '' || window.location.pathname === '/') {
            const fallbackSection = sections[0]
            if(fallbackSection) {
                setNextSectionId(fallbackSection.id)
                // Update URL without hash for clean routing
